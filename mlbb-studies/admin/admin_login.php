...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$username || !$password) {
        ...
        $stmt = $mysqli->prepare("SELECT id, username, password_hash FROM admins WHERE username=? OR email=?");
        $stmt->bind_param('ss', $username, $username);
        $stmt->execute();
        $stmt->store_result();
        if ($stmt->num_rows === 1) {
            $stmt->bind_result($id, $adminUser, $hash);
            $stmt->fetch();
...
<head>
  <meta charset="UTF-8">
  <title>Admin Login - MLBB Studies</title>
...
<body>
  ...
  <div class="login-card">
    <div class="text-center">
        <img src="/mlbb-studies/logo/logo-v2.png" class="logo" alt="Admin Logo">
    ...
    <form method="post" autocomplete="off" novalidate>
        ...
        <button type="submit" class="btn btn-primary w-100 py-2 mt-2">Login as Admin</button>
    </form>
  </div>
